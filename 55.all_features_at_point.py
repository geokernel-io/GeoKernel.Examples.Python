...
from PySide6.QtWidgets import QApplication, QHBoxLayout, QMainWindow, QMessageBox, QTableWidget, QTableWidgetItem, QToolBar, QWidget
from geokernel import Extent, Viewer, ViewerEventType, ViewerTool
from common import application_icon, ensure_sample_file

class AllFeaturesAtPointWindow(QMainWindow):
    def __init__(self, app: QApplication) -> None:
        ...
        self.setWindowTitle("AllFeaturesAtPoint")
    ...
    def initialize_viewer(self) -> None:
        ...
        try:
            ...
            self.statusBar().showMessage(
                "Click the map to list all features under the pointer."
            )
        except Exception as error:
            QMessageBox.critical(self, "AllFeaturesAtPoint", str(error))
    ...
    def _query_hits(self, x: int, y: int):
        hits = self.viewer.hit_test_features_at(x, y, 8)
        if hits:
...
def main() -> None:
    ...
    app.setApplicationName("AllFeaturesAtPoint")
    app.setWindowIcon(application_icon())
    window = AllFeaturesAtPointWindow(app)
    window.show()
