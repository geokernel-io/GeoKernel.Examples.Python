...
class KmlLoadWindow(QMainWindow):
    ...
    def initialize_viewer(self) -> None:
        ...
        self.viewer.show()
        try:
            kmz_path = ensure_sample_file(
                app=self.app,
                zip_url=(
                    "https://github.com/geokernel-io/GeoKernel.SampleData/"
                    "releases/download/v1/travel.zip"
                ),
                zip_name="travel.zip",
                target_folder="travel_kml",
                required_file="travel.kmz",
                title="KmlLoad",
            )
            path = self.ensure_kml_document(kmz_path)
            self.viewer.add_layer(str(path))
            self.populate_details(path)
            self.viewer.zoom_to_layer(0)
        except Exception as error:
            QMessageBox.critical(
                self, "KmlLoad", f"Layer could not be loaded:\n\n{error}"
            )

    @staticmethod
    def ensure_kml_document(kmz_path: Path) -> Path:
        target_directory = kmz_path.parent / "travel_kml_content"
        existing = (
            next(target_directory.rglob("doc.kml"), None)
        ...
        with ZipFile(kmz_path) as archive:
            for member in archive.infolist():
                ...
                if (
                    target_root not in destination.parents
                    and destination != target_root
                ):
                    raise RuntimeError(f"Unsafe KMZ entry: {member.filename}")
            archive.extractall(target_directory)

        document = next(target_directory.rglob("doc.kml"), None)
        if document is None:
            raise FileNotFoundError(f"KMZ archive does not contain doc.kml: {kmz_path}")
        return document
    ...
    def populate_details(self, path: Path) -> None:
        ...
        self.statusBar().showMessage(f"KmlLoad loaded: {path.name}")
...
def main() -> None:
    ...
    app.setApplicationName("KmlLoad")
    app.setWindowIcon(application_icon())
    window = KmlLoadWindow(app)
    window.show()
