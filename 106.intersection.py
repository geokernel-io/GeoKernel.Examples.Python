...
class IntersectionWindow(QMainWindow):
    ...
    def create_ui(self) -> None:
        ...
        toolbar_layout.addWidget(
            QLabel("Operation: Intersection(left, right)", toolbar)
        )

        run_button = QPushButton("Run Intersection", toolbar)
        run_button.clicked.connect(self.run_intersection)
    ...
    def initialize_viewer(self) -> None:
        ...
        try:
            ...
            self.statusBar().showMessage("Intersection initialization failed.")

    def run_intersection(self) -> None:
        if self.initialized:
    ...
    def render_scene(self, show_result: bool) -> None:
        ...
        if not show_result:
            details.append("Result: click Run Intersection to calculate")
            self.statusBar().showMessage(
                "Source polygons are ready. Click Run Intersection."
            )
        else:
            result = self.viewer.intersection_polygons(LEFT_POLYGON, RIGHT_POLYGON)
            rings = [
            ...
            if not rings:
                ...
                self.statusBar().showMessage("Intersection returned an empty result.")
            else:
                for ring in rings:
                    if not self.viewer.add_polygon_shape(
                        ...
                        raise RuntimeError(
                            "Intersection result shape could not be rendered."
                        )
                details.extend(
                    (
                        "Result type: polygon",
                ...
                self.statusBar().showMessage("Intersection result created.")
...
def main() -> None:
    ...
    app.setApplicationName("Intersection")
    app.setWindowIcon(application_icon())
    window = IntersectionWindow()
    window.show()
