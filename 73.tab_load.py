...
class TabLoadWindow(QMainWindow):
    ...
    def initialize_viewer(self) -> None:
        ...
        try:
            path = ensure_sample_file(
                app=self.app,
                zip_url="https://github.com/geokernel-io/GeoKernel.SampleData/releases/download/v1/paris_tab.zip",
                zip_name="paris_tab.zip",
                target_folder="paris_tab",
                required_file="paris.tab",
                title="TabLoad",
            )
            ...
            QMessageBox.critical(
                self, "TabLoad", f"Layer could not be loaded:\n\n{error}"
            )
    ...
    def populate_details(self, path: Path) -> None:
        ...
        self.statusBar().showMessage(f"TabLoad loaded: {path.name}")
...
def main() -> None:
    ...
    app.setApplicationName("TabLoad")
    app.setWindowIcon(icon)
    window = TabLoadWindow(app)
    window.show()
