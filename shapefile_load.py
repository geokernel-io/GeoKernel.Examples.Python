...
class ShapefileLoadWindow(QMainWindow):
    def __init__(self, app: QApplication) -> None:
        ...
        self.setWindowTitle("ShapefileLoad")
    ...
    def initialize_viewer(self) -> None:
        ...
        self.viewer.show()
        try:
            path = ensure_sample_file(app=self.app, zip_url="https://github.com/geokernel-io/GeoKernel.SampleData/releases/download/v1/world_4326.zip", zip_name="world_4326.zip", target_folder="world_4326", required_file="world_4326.shp", title="ShapefileLoad")
            self.viewer.add_layer(str(path))
            self.populate_details(path)
            self.viewer.zoom_to_layer(0)
        except Exception as error:
            QMessageBox.critical(self, "ShapefileLoad", f"Layer could not be loaded:\n\n{error}")
    ...
    def populate_details(self, path: Path) -> None:
        ...
        self.details.setPlainText("ShapefileLoad sample\n\nAPI\nadd_layer(path)\nlayer_info(index)\nlayer_attribute_definitions(index)\n\nLoaded: " + str(path) + "\n\nLayer\n" + str(info))
        ...
        self.statusBar().showMessage(f"ShapefileLoad loaded: {path.name}")
...
def main() -> None:
    ...
    app.setApplicationName("ShapefileLoad")
    app.setWindowIcon(icon)
    window = ShapefileLoadWindow(app)
    window.show()
