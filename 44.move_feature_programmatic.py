...
class MoveFeatureProgrammaticWindow(QMainWindow):
    ...
    def create_ui(self) -> None:
        ...
        self.select_action = self.create_action("Select.svg", "Select", self.toggle_select, True)
        ...
        self.west_action = self.create_action("Move.svg", "Move West", self.move_west)
        self.east_action = self.create_action("Move.svg", "Move East", self.move_east)
        self.north_action = self.create_action("LayerUp.svg", "Move North", self.move_north)
        self.south_action = self.create_action("LayerDown.svg", "Move South", self.move_south)
        for action in (self.west_action, self.east_action, self.north_action, self.south_action):
            toolbar.addAction(action)
        toolbar.addSeparator()
        self.reset_action = self.create_action("Refresh.svg", "Reset Points", self.populate_points)
