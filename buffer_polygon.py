...
FULL_EXTENT = Extent(-6.0, -4.0, 6.0, 4.0)

class BufferPolygonWindow(QMainWindow):
    def __init__(self) -> None:
        ...
        self.setWindowTitle("Buffer Polygon")
    ...
    def create_ui(self) -> None:
        ...
        self.distance_spin.setValue(0.60)
    ...
    def initialize_viewer(self) -> None:
        ...
        try:
            ...
            self.statusBar().showMessage("BufferPolygon initialization failed.")
    ...
    def update_buffer(self, distance: float | None = None) -> None:
        ...
        buffer_added = self.viewer.add_polygon_buffer_shape(
            SOURCE_POLYGON,
            buffer_distance,
            12,
            {
        ...
        if not buffer_added:
            ...
            self.add_source_polygon()
            return

        self.add_source_polygon()
        source_extent = self.source_extent()
        result_extent = source_extent.inflate(buffer_distance, buffer_distance)
        self.viewer.invalidate_render_cache(True, True)
        self.viewer.refresh_layers()
        self.details_view.setPlainText(
            "\n".join(
                (
                    "MakeBuffer(polygon, distance)",
                    "Source parts: 1",
                    f"Source vertices: {len(SOURCE_POLYGON)}",
                    f"Distance: {buffer_distance:.2f} map units",
                    "Result type: polygon",
                    "Result parts: 1",
                    f"Source extent: {self.extent_text(source_extent)}",
                    f"Result extent: {self.extent_text(result_extent)}",
                )
            )
        )
        self.statusBar().showMessage(
            f"Polygon buffer distance: {buffer_distance:.2f} map units"
        )

    def add_source_polygon(self) -> None:
        ...
        if not added:
            raise RuntimeError("Source polygon shape could not be created.")

    def source_extent(self) -> Extent:
        x_values = [point[0] for point in SOURCE_POLYGON]
        y_values = [point[1] for point in SOURCE_POLYGON]
        return Extent(
            min(x_values),
            min(y_values),
            max(x_values),
    ...
    def show_empty_result(self, distance: float) -> None:
        self.details_view.setPlainText(
            f"MakeBuffer(polygon, {distance:.2f}) returned an empty shape."
        )
...
def main() -> None:
    ...
    app.setApplicationName("BufferPolygon")
    app.setWindowIcon(application_icon())
    window = BufferPolygonWindow()
    window.show()
