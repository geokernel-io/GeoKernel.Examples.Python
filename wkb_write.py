...
class MousePressFilter(QObject):
    def __init__(self, window: "WkbWriteWindow") -> None:
...
class WkbWriteWindow(QMainWindow):
    ...
    def initialize_viewer(self) -> None:
        ...
        try:
            ...
            self.statusBar().showMessage("WkbWrite initialization failed.")
    ...
    def create_edit_layers(self) -> None:
        ...
        for mode, style in zip(MODES, styles):
            index = self.viewer.add_empty_vector_layer(mode[1], mode[2], style)
            if index < 0:
    ...
    def resolve_layer_indexes(self) -> None:
        ...
        for index, layer in enumerate(self.viewer.layers_info()):
            ...
            if name in names:
                self.layer_indexes[name] = index
        if len(self.layer_indexes) != len(MODES):
            raise RuntimeError("Editable WKB layers could not be resolved.")
    ...
    def activate_selected_mode(self) -> None:
        ...
        if index < 0:
            return
        if not self.viewer.is_layer_editing(index):
            if not self.viewer.begin_edit_layer(index):
    ...
    def handle_map_mouse_press(self) -> None:
        ...
        starts_new_geometry = mode_index == 0 or not self.drawing_sketch
        if not starts_new_geometry:
            return
    ...
    def on_viewer_event(self, event) -> None:
        ...
        if layer_index < 0 or self.viewer.layer_feature_count(layer_index) == 0:
            return
        self.drawing_sketch = False
        self.write_active_wkb()

    def write_active_wkb(self) -> None:
        mode_index = self.mode_combo.currentIndex()
        layer_index = self.active_layer_index()
        wkb = self.viewer.write_layer_last_shape_wkb(layer_index)
        if not wkb:
            self.show_empty_details()
            self.statusBar().showMessage(
                "No drawn geometry is available for the selected type."
            )
            return

        api_name = self.api_name(mode_index)
        hex_view = " ".join(f"{byte:02X}" for byte in wkb)
        self.details_view.setPlainText(
            "\n".join(
                (
                    ...
                    self.active_mode()[0],
                    "Layer feature count: "
                    f"{self.viewer.layer_feature_count(layer_index)}",
                    "",
                    "Output WKB",
                    f"Byte count: {len(wkb)}",
                    ...
                    "3. WKB binary is written automatically when drawing finishes.",
                )
            )
        )
        self.statusBar().showMessage(f"{api_name} wrote {len(wkb)} WKB bytes.")
    ...
    def api_name(self, mode_index: int) -> str:
        return (
            "GisWkbWriter::writePoint(shape)",
            "GisWkbWriter::writePolyline(shape)",
            "GisWkbWriter::writePolygon(shape)",
        )[mode_index]

    def help_text(self, mode_index: int) -> str:
        return (
            "Click on the map to draw a point. WKB is written automatically.",
            "Click line vertices, then press Enter or double-click to finish. "
            "WKB is written automatically.",
...
def main() -> None:
    ...
    app.setApplicationName("WkbWrite")
    app.setWindowIcon(application_icon())
    window = WkbWriteWindow()
    window.show()
