...
class MifLoadWindow(QMainWindow):
    ...
    def initialize_viewer(self) -> None:
        ...
        try:
            path = ensure_sample_file(
                app=self.app,
                zip_url="https://github.com/geokernel-io/GeoKernel.SampleData/releases/download/v1/albania.zip",
                zip_name="albania.zip",
                target_folder="albania",
                required_file="albania.mif",
                title="MifLoad",
            )
            ...
            QMessageBox.critical(
                self, "MifLoad", f"Layer could not be loaded:\n\n{error}"
            )
    ...
    def populate_details(self, path: Path) -> None:
        ...
        self.statusBar().showMessage(f"MifLoad loaded: {path.name}")
...
def main() -> None:
    ...
    app.setApplicationName("MifLoad")
    app.setWindowIcon(icon)
    window = MifLoadWindow(app)
    window.show()
