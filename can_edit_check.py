...
class CanEditCheckWindow(QMainWindow):
    ...
    def create_ui(self) -> None:
        ...
        self.addToolBar(bar)
        self.begin_action = self.make("Edit.png", "Begin Edit", self.begin_edit)
        self.commit_action = self.make("Save.png", "Commit Edit", self.commit)
        self.rollback_action = self.make("Rollback.png", "Rollback Edit", self.rollback)
        self.select_action = self.make("Select.png", "Select", self.toggle_select, True)
        self.clear_action = self.make(
            "Delete.png", "Clear Selection", self.clear_selection
        )
        self.reset_action = self.make(
            "Refresh.png", "Reset Points", self.populate_points
        )
