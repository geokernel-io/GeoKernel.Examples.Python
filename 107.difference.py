...
LEFT_POLYGON = [(-4.4, -1.8), (1.2, -1.8), (1.2, 2.2), (-4.4, 2.2), (-4.4, -1.8)]
RIGHT_POLYGON = [
    (-0.2, 3.0),
    (0.6, 1.2),
    (3.2, 1.2),
...
class DifferenceWindow(QMainWindow):
    ...
    def create_ui(self) -> None:
        ...
        toolbar_layout.addWidget(QLabel("Operation: Difference(left, right)", toolbar))
        run_button = QPushButton("Run Difference", toolbar)
        run_button.clicked.connect(self.run_difference)
    ...
    def initialize_viewer(self) -> None:
        ...
        try:
            ...
            self.statusBar().showMessage("Difference initialization failed.")
    ...
    def render_scene(self, show_result: bool) -> None:
        ...
        if not show_result:
            details.append("Result: click Run Difference to calculate")
            self.statusBar().showMessage(
                "Source polygons are ready. Click Run Difference."
            )
        else:
            result = self.viewer.difference_polygons(LEFT_POLYGON, RIGHT_POLYGON)
            rings = [
            ...
            if not rings:
                ...
                self.statusBar().showMessage("Difference returned an empty result.")
            else:
                ...
                if not added:
                    raise RuntimeError("Difference result shape could not be rendered.")
                details.extend(
                    (
                        "Result type: polygon",
                ...
                self.statusBar().showMessage("Difference result created.")
    ...
    def extent_text(self, extent: Extent) -> str:
        return f"({extent.x_min:.2f}, {extent.y_min:.2f}) - ({extent.x_max:.2f}, {extent.y_max:.2f})"
...
def main() -> None:
    ...
    app.setApplicationName("Difference")
    app.setWindowIcon(application_icon())
    window = DifferenceWindow()
    window.show()
